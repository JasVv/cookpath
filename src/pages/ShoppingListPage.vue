...
<template>
  <section class="max-w-3xl">
    ...
    <div v-if="rows.length === 0 && supplyRows.length === 0" class="text-sm text-slate-500 py-8 text-center">
      指定期間内に材料が登録された料理がなく、日用品も登録されていません。
    </div>

    <div v-else class="space-y-6">
      <div>
        <h3 class="text-sm font-semibold text-slate-700 mb-2">食材</h3>
        <div v-if="rows.length === 0" class="text-sm text-slate-500 py-4 text-center bg-white border border-slate-200 rounded">
      ...
      <div v-if="supplyRows.length > 0">
        <h3 class="text-sm font-semibold text-slate-700 mb-2">日用品</h3>
        <ul class="bg-white border border-slate-200 rounded divide-y divide-slate-100">
          <li
